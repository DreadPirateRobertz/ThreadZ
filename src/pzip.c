	  ...
	  int *char_frequency)
{
	...
	struct threadInfo tInfo = { n_threads,		input_chars,
				    input_chars_size,	zipped_chars,
...
void *callBack(void *arg)
{
	...
	int charCount = 1;
	struct zipped_char *local =
		malloc(sizeof(struct zipped_char) * allotment);
	...
	for (int i = start; i < length; i++) {
		...
		if (currChar == nextChar && (i < (length - 1))) {
			...
			master->frequency[local[index].character - 97] +=
				charCount;
			pthread_mutex_unlock(
				&fLock[local[index].character - 97]);
			index++;
			charCount = 1;
		}
	}
